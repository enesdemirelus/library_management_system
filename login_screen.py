...
class LoginScreen(tk.Tk):
    def __init__(self):
        ...
        cur.execute("SELECT id FROM userdatabase")
        self.id_numbers = [row[0] for row in cur.fetchall()]
        if self.id_numbers:
        ...
        if username and password:
            if " " in username:
                ...
                if username in self.member_names:
                    cur.execute("SELECT password FROM userdatabase WHERE name = ?", (username,))
                    saved_password = cur.fetchall()[0][0]
                    cur.execute("SELECT isadmin FROM userdatabase WHERE name = ?", (username,))
                    adminValue = cur.fetchall()[0][0]
        ...
        if username and password:
            if " " in username:
                ...
                if username not in self.member_names:
                    cur.execute('INSERT INTO  userdatabase (id, name, password, isadmin) VALUES (?, ?, ?, ?)', (id, username, password, self.radio_button_var.get()))
                    con.commit()
